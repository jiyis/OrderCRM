...
class traverseDir{
    ...
    public $savepath;
    public $zipname;
    public function __construct($curpath,$savepath,$zipname){
        $this->currentdir=$curpath;//返回当前目录
        $this->savepath=$savepath;//返回当前目录
        $this->zipname=$zipname;//返回当前目录
    }        
    //遍历目录
    ...
    public function tozip($items){
        ...
        if (!file_exists($this->zipname)){
            $zip->open($savepath.$this->zipname.'.zip',ZipArchive::OVERWRITE);//创建一个空的zip文件
            for ($i=0;$i<count($items);$i++){
                $zip->addFile($this->currentdir.'/'.$items[$i],$items[$i]);
            }
            $zip->close();
            $dw=new download($this->zipname.'.zip',$savepath); //下载文件
            $dw->getfiles();
            unlink($savepath.$this->zipname.'.zip'); //下载完成后要进行删除
        }
    }
}
